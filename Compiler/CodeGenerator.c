...
void GenerateIntermidiateCode(Node * ast, FILE* logFile, Element* datacode, Element* progcode, SymbolToken * table)
{
	...
	if (ast->type == OP_ADD || ast->type == OP_MINUS || ast->type == OP_MUL || ast->type == OP_DIV) {
		...
		else if (ast->kids[1]->type == LITERAL_NUMBER)
		{
			...
			if (ast->kids[0]->type == IDENTIFIER) {
				...
				char bufferdec[100];
				sprintf(bufferdec, "t%d", registerCounter - regsub);
				insertSymbolToken(IDN_NUMBER, ast, bufferdec, table);//TODO: DECIDE THE TYPE
				strcpy(bufferdec, "");
				sprintf(buffer, "	t%d : dd 0 \n", registerCounter - regsub);
				datacode = InsertList(datacode, buffer);
				strcpy(buffer, "");
	...
	{
		if (ast->kids[1]->type == LITERAL_NUMBER || ast->kids[1]->type == LITERAL_DECIMAL ||
	...
	{
		if (ast->kids[0]->type == IDN_NUMBER || ast->kids[0]->type == IDN_DECIMAL || ast->kids[0]->type == IDN_CHAR || ast->kids[0]->type == IDN_BOOL)
		{
			strcpy(buffer, "");
		...
		if (ast->kids[0]->type == RESERVED_CONSOLE) {
			if (ast->kids[1]->type == IDENTIFIER)
			{
				...
				printf("		push formatoutnumber; push message into ESP\n");
				fprintf(logFile, "		push formatoutnumber; push message into ESP\n");
			...
			else if (ast->kids[1]->type == RESERVED_BOOL_FALSE || ast->kids[1]->type == RESERVED_BOOL_FALSE)
			{
				...
				strcpy(buffer, "");
				sprintf(buffer, "		push formatoutnumber; push format into ESP\n");
				progcode = InsertList(progcode, buffer);
